...
Read the critique **also in the context of the canonical source-of-intent doctrine** — the operator-side **Source of Intent** master (`control-surface-EXTERNAL/source-of-intent_master-context-note.md`) and its **Image-Making + Source of Intent** branch (`control-surface-EXTERNAL/image-making-source-of-intent_master-context-note.md`), or the mounted/uploaded equivalents on a tool surface without filesystem access. These carry the canonical source-of-intent primitive — **normative apex >> governance architecture >> operational floor >> execution substrate** — and its image-making application; they are the reference the ecology's source-of-intent, authorship, and governance claims are expected to be coherent with, and the working lens for the second lens below. Context for the critique, not itself a critique target; the graduated public form lives in `method-ASK/docs/` (`normative-apex.md` · `governance.md` · `source-of-intent.md` · `bounded-generativity.md`).
