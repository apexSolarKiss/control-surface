# Repo Critique Initial Prompt // Structured

Status: non-default structured critique-cycle prompt.

Use this prompt to invoke a structural critique of a repo against its grounding note with an explicit boundary-classification block at the end. This is the heavier variant of the critique-initial prompt; the minimal default lives in `repo-critique-initial-prompt.md`. Pair with `repo-critique-synthesis-prompt.md` to fold independent critiques into a plan.

The cycle is warranted at thresholds where the project's structure, posture, or direction needs re-examination — not as a standing review.

---

Review the current state of the repo and surface high-level observations — concerns, doubts, or things that don't make sense — anchored in the grounding note and the higher-level purpose articulated therein.

Rely on durable truth:

- the repo as it currently is (read it directly)
- the grounding note maintained outside the repo

Do not rely on:

- thread history or accumulated explanations from prior sessions
- operator summaries or narrative framing of the project's state

The point is context reset: reconstruct the project from durable truth and surface what reads unclear, contradictory, overclaimed, stale, or structurally awkward. Open-ended is the posture; the durable backbone does the focusing.

This is a critique, not a plan. Produce observations, not corrective actions.

Also identify what additional external operator source of intent or direction is needed next, if any. Distinguish:

- missing intent (the operator must supply a new purpose-level decision)
- unresolved means (the premise is supplied; an architecture attempt or pressure test is implied)
- sequencing choices among valid routes
- repo-local absorption or routing
- evidence-depth assessments
- need for further fresh-context critique

When the premise is already supplied by the durable sources, do not flag it as missing intent.
