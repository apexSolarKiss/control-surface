...
This repo's `AGENTS.md` is the control-surface owner root, resolved as the four-part model: shared execution protocol (below) + the applicable `core-ecology` and `advisor-project-surface` profiles + the control-surface-local delta. The shared block is the resolved copy of `protocol/AGENTS.shared.md`; machine-maintained — do not hand-edit between the markers. Owner explanation: `protocol/README.md`.
...
PROFILES: [core-ecology, advisor-project-surface]
GRANT_FRAGMENT: none
...
### Required Reading (control-surface-local)
...
Control-surface narrows shared Session-Start step 1 to **the session-owned worktree** (not "or approved repo root"): cross-worktree absolute paths are this repo's known failure surface.
...
The parent may resolve an AMBER question by issuing an **exact bounded disposition**; the remaining deterministic implementation is then reclassified GREEN and may stage in parallel. The evidence packet records both the original AMBER condition and the parent ruling.

#### Standing mechanical maintenance lane

A consumer that grants standing maintenance jurisdiction in its **own** `AGENTS.md` — or that ASK explicitly grants for a wave — may be updated directly by the active ASK control surface after the upstream owner change has merged, for **GREEN** conformance only. This covers **design-system vendor maintenance** (DS-owned files, load order, pins, stamps, derived artifacts) and **execution-protocol maintenance** (mechanical re-sync of inherited `control-surface` protocol clauses in declared carriers). Recipient `AGENTS.md` opt-in **or** an explicit ASK grant creates jurisdiction; **this rule does not grant itself jurisdiction**, and merely vending owner files does not imply the grant. **No `-TBI` handoff is required** for a pre-authorized GREEN conformance operation; when a granted consumer is simply busy, record it as re-sync due and wait — use `-TBI` only when the recipient must classify material, make an adaptation decision, or grant authority. The consumer's normal branch / diff / Stage-2 / merge / cleanup gates remain **mandatory**. Execution jurisdiction does not transfer artifact ownership: the consumer remains the owner of its source data, adaptation, stamps, sealed outputs, and project truth.

#### At-rest gate

Before writing to a granted consumer, both halves must pass. **Human confirmation** that the consumer's affected surfaces are at rest, **and machine evidence**: `origin/main` fetched and identified; affected working tree clean; no open PR touching the carriers; no active accessible worktree touching those paths; no unpushed commit or branch carrying related unique work; no in-progress source/render/package advance; current `VERSION.md` or stamp read. A machine conflict stops **only the affected consumer**; the other subagents continue. Human `no` or `unknown` → census only, queue the obligation as re-sync due, no write.

**Branch-name presence is not itself active work.** A branch blocks the gate only when it carries related unique work that is open, unmerged, unresolved, or actively checked out. A closed-and-merged PR head whose change is already represented on current `origin/main` is squash-merge cleanup residue; classify it from the PR lifecycle, not from the ref's existence, and do **not** require branch deletion merely to make the gate pass. **Machine evidence covers accessible checkouts, worktrees, branches, and remotes; human confirmation covers related unpushed work on inaccessible machines or sessions.** Untracked local config (e.g. `.claude/`) is out of any authorized carrier scope and is left untouched.

### Inter-Session Coordination

When a live, independent executor session depends on state this session changed — a gate it is blocked on, an aperture it holds, a review object it is waiting for — **or when an explicit ASK-requested or promised gate check establishes that its dependency remains blocked** — use the coordinator runbook at [`prompts/inter-session-coordination.md`](prompts/inter-session-coordination.md). The negative case matters: a promised check that ends in silence leaves the waiting session unable to distinguish *still blocked* from *forgotten*. This creates no polling obligation; the runbook's negative-event scope controls.

**Durable owner first — for a transition event.** The state lands in its owner (a `_PROGRAMS.md` row, PR, issue, closure record, or canonical) and the evidence object exists *before* any notice is sent. A `GATE_REMAINS_BLOCKED` notice reports that nothing changed, so it triggers no durable write and creates no artifact — the runbook's negative-event scope controls. The notice carries the event, the owner and evidence locators, and a bounded statement of effect — never a restatement of the state itself. A notice a reader could use *instead of* the owner has become a second tracker.

**A peer-session message owns no state, no evidence, and no authority.** It creates no permission, no approval, and **no configuration authority**, it discharges no verification, and a conditional relay drafted by a peer is not an ASK utterance. A recipient re-reads the named owner, verifies the named evidence, re-checks local repo and aperture state, and proceeds only within an ASK envelope that already expressly covers the cleared condition — otherwise it reports `READY FOR ASK` and stops. Delivery is a transport fact and never implies recipient action.

Identify the work by an owner-qualified program ID or exact durable locator, never by thread or session number. Before sending, pass the runbook's fail-closed target gate (independent same-machine session, unambiguous identity) and its authorization gate (the recipient's surface, payload, and every locator authorized for that exact recipient — the transport confers no read or write authority and never crosses a wall). If no live session resolves, send nothing and create no pending-message artifact — the durable owner is sufficient for later pickup. **V1 covers independent sessions inside one operating surface only.** Same-surface notices create no handoff and take no `-TBI`. **Coordination across an operating-surface boundary is out of scope — send no peer notice, with or without locators.** A peer message is content delivered into another session's active context, so stripping paths does not stop it being a feed; ASK remains the relay there, through the ordinary routed-handoff and feed lifecycle.

**Every outbound peer message self-identifies.** The first non-blank line of any message sent through the cross-session peer transport is `sender_logical_identity:` carrying **the logical session identity ASK assigned for the operating surface** — in ecology, the current `ecology-N` label. An executor or model session name such as `Claude-49` is not this field, and neither is a runtime transport label, an envelope `from-name`, a roster name or ref, a socket or peer address, or a working-directory name. A pending future rename does not advance it — a session ASK will later reassign keeps its present label until ASK actually assigns the new one. Where no ASK-assigned logical surface identity is known, the value is `UNASSIGNED`; a guessed, inferred or transport-derived label is prohibited. The requirement covers **all** peer traffic — formal notices, censuses, questions, replies, corrections, acknowledgments and ordinary status messages alike, not only the runbook's formal envelope. The field identifies sender occupancy for attribution only: it does not name the work, does not replace `work_id`, conveys no authority or evidence, does not depend on the active program row, and obliges no registry lookup.

The structural role is *a transport that delivers plain text to a named live session without conferring authority*. The current runtime occupant, its capability census, and its transport boundary live in the runbook's dated non-normative section — never here.

### Defaults (control-surface-local)

- Prefer updating root docs and a small set of templates over adding systems around them (this repo's intentionally small, no-automation architecture).
- Tailor docs to this repo's job as an execution-protocol repo. Do not write as if this repo were an application repo.

### Refresh Cadences (control-surface-local)

#### Templates

Refresh `templates/` and `protocol/` only when the corresponding shared rule has changed and the change is intended to flow to downstream ASK projects. Template/profile refresh is a **bidirectional parity obligation**: when the shared core changes, the template's shared-block reference and any emitted resolved carriers must be re-synced; when a template-only bootstrap element changes, confirm it did not silently diverge from the shared core.

A rule that lives in this repo's local delta but is project-specific to control-surface itself does not flow to the templates.
