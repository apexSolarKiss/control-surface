...
## Standing upstream conformance grants (optional — opt-in only)

> **Include this section only if this repo opts in.** Merely vending design-system-ASK files, or inheriting control-surface protocol, does **not** imply the grant — a downstream repo grants standing jurisdiction explicitly, here, or not at all. Delete this whole section if the repo has not opted in; ASK routes each wave individually in that case.
