...
- **Return transport** — if a return requires a physical write, the provider and the **one exact destination**; otherwise `none — direct relay only`. Declared before first use, never inferred from the return mode.
- **Transport grant bounds** — only where a transport exists: the confirmation trigger that permits a write, the admissible payload, collision behavior (target exists → stop and ask), the receipt returned (path, byte size, provider file ID where one exists), and the fallback when the provider is unavailable. Create-only — no overwrite, update, rename, move, delete, folder creation, or link creation. Transport only: no ingestion, absorption, adoption, implementation, publication, or closure authority.
...
- **Transport write required?** — whether this return actually needs the standing grant exercised, and if so, the confirmed body that is admissible. A return mode does not imply a write.
...
*The invariants that govern how this profile is used — authority declaration, expertise-is-not-project-authority, no implicit stage advancement, claim-level classification, the handoff-necessity gate, direct-relay sufficiency, exact-wording preservation, advisor/generated-thread non-authorization, routing, and return transport as a declared create-only grant — live in `apexSolarKiss/control-surface/docs/domain-authority-review-protocol.md`. This template is the fill-in; the protocol is the rule.*
