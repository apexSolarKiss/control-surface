...
**Consequence for Project Sources.** Do **not** remount operator-side canonicals into the GPT Project's Sources by default. When the connector exists, **Dropbox is the preferred live read path for operator-side canonicals** — point the model at the canonical Dropbox path and have it fetch the current file directly. The Project's Sources are for **bootstrap, fallback, and connector-failure resilience** (a small read-path/bootstrap note; the active project's grounding note if it is the first routing object; anything that must survive a connector outage; a package source only when Dropbox cannot reach it) — **not** for routine canonical mirroring. This removes the mount-refresh burden: canonicals stay current at their Dropbox path, not in a mounted mirror.
