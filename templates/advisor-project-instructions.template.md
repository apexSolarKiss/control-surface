...
You work on a **live read of the operator's filesystem** — connector fetches show current on-disk state, and your session memory is a **stale base** against it. Before proposing or cutting a **new version** of anything (memo vN, draft vN, canonical + `_vN` snapshot, grounding note):
...
- **observe the filename conventions** on save-ready output: dated scratch names (`YYYY-MM-DD …`) · `Title vN.md` is a frozen snapshot (any change cuts `vN+1`, never edit-in-place) · canonical-unversioned notes edit in place + cut `_vN` · `-TBI` marks handoff intake, and renamed-off-`-TBI` means already ingested — if your `-TBI` target is renamed or gone, route a **new** memo rather than updating the old one;
- **scratch files may include operator hand-assembled provenance records** — often short dated filenames, sometimes dialogue-marker transcripts or `v0`/`vN` files. Do not assume they are model drafts. Before cutting a new version or superseding a file, classify the artifact role from the file itself and nearby scratch context: provenance transcript, model draft, absorption memo, routed handoff, closure record, or snapshot;
