...
This project follows `method-ASK/docs/source-of-intent.md` §Inbound handoff TBI marker. Inbound handoff memos arriving in this project's external intent inbox — at the exact live path its `_INDEX` declares — may carry a terminal `-TBI.md` overlay, ASK's outstanding feed-obligation marker (the *current* feed obligation is unsatisfied; not a statement about disposition, and not evidence the artifact was never ingested before). The repo-local execution form, including the full state machine, lives in `AGENTS.md` §Inbound Handoff TBI Marker.
