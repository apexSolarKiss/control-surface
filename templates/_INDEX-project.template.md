...
| intent inbox (intake) | `S/<project>-EXTERNAL/<the surface's current live intake path>` | candidate / ingestion — the **filename lifecycle marker** carries current disposition (`-TBI` terminal feed-obligation overlay · `-ingested` read-but-not-closed · terminal disposition suffix · `-supersededA` retired before ingestion · `-supersededP` ingested-then-displaced); the folder alone does not establish queue membership. Post-cutover convention is `intent-INbox/`; **the live path above governs until this surface's cutover is recorded** |
