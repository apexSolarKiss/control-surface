...
**Mount posture.** In healthy connector mode this file is **not mounted**. The advisor Project's single standing Markdown Source is that surface's bootstrap (`templates/advisor-project-bootstrap.template.md`); the bootstrap carries this index's exact locator and fetches it live, so an ordinary index revision needs no remount. Mount a copy only as connector-failure fallback, and retire it when live access returns. **A mount receipt proves identity from the mounted body, and the fields available depend on which carrier is mounted: three operating modes across two carrier families** — carrier families bootstrap-mounted and index-mounted; operating modes healthy bootstrap, connector-failure fallback, and A10 rollback. **Healthy mode (bootstrap receipt):** standing-source cardinality (exactly one, and this index not among the mounts); expected surface identity read from the mounted bootstrap — its expected H1 / role and the exact `INDEX_CANONICAL_LOCATOR` naming *this* index, since another surface's current bootstrap satisfies count and version while pointing at a different index; and the intended revision, proven by in-body version **and** a distinguishing clause, in a **fresh thread**. In either of the two mounted-index modes below this file is the mounted carrier, and it holds neither an `INDEX_CANONICAL_LOCATOR` field nor a bootstrap version banner — so verify it by what it does carry. **The two modes are different operating states and must not be merged.** **Connector-failure fallback (temporary):** the thin Instructions field is unchanged and the bootstrap stays mounted, so the expected Source set is bootstrap + this fallback copy. Prove this index's own H1 / surface / role plus one distinguishing mapped path or clause in a fresh thread; **no full Instructions canonical is repasted and no rollback is invoked or inferred**; retire the copy once live access returns. **A10 rollback (architecture change):** the frozen full Instructions canonical is repasted and the frozen index mounted, and **both evidence planes are required** — *operator-side installation evidence:* the expected rollback Source set and cardinality, the exact frozen full Instructions canonical identity, and the exact frozen index identity where byte identity is load-bearing; *fresh-thread evidence:* this index's own H1 / surface / role plus one distinguishing mapped path or clause, and the restored behavior. **A fresh thread cannot establish what was pasted into the Instructions field** — a behavioral pass is never proof of the repasted canonical's bytes. Mounted-source display labels are inspection metadata, not source identity: supporting evidence for selection only in any mode, never sufficient for cardinality, expected identity, or revision, and a host-added `(N)` decoration proves neither duplication nor wrong bytes. The placement contract is owned by [`docs/advisor-project-surface-architecture.md`](../docs/advisor-project-surface-architecture.md).
...
| intent inbox (intake) | `S/<project>-EXTERNAL/<the surface's current live intake path>` | candidate / ingestion — **two axes**: the **underlying filename** carries role plus the primary durable-state marker (`-ingested` read-but-not-closed · terminal disposition suffix · `-supersededA` retired before ingestion · `-supersededP` ingested-then-displaced), while **terminal `-TBI` is independent feed-obligation evidence — not a disposition, and excluded from the disposition-agreement check**. A disposition or lineage record must agree with the **underlying durable-state marker**, never with terminal `-TBI`. The folder alone does not establish queue membership. Post-cutover convention is `intent-INbox/`; **the live path above governs until this surface's cutover is recorded** |
| inbox state *(post-cutover only)* | `S/<project>-EXTERNAL/intent-INbox/_STATE.md` | structural — **not routed intent**: no lifecycle suffix, excluded from queue counts, updated in place, read immediately before ingestion. **`OPEN`** = no additional hold, ordinary governed ingestion proceeds · **`FROZEN`** = no routed artifact ingested unless its exact filename is an exception · **`PARTIAL-HOLD`** = the named held scope is blocked absent exact-filename exceptions, unrelated scope stays governed normally. Exceptions are exact filenames or explicitly `NONE`; the record also carries scope, the ASK authorization locator, effective time, and a review trigger. A structural artifact is exempt only where this contract **names** it — a leading `_` alone confers nothing. Before cutover its absence is not nonconformance; once declared active, unreachable or malformed state **fails closed for ingestion** |
...
- **Path resolution ≠ content read. Successful content read into active context *is* ingestion. Ingestion ≠ disposition / absorption / canonicalization / promotion. Content read ≠ exact-byte identity proven.** Header + path govern status (scratch stays scratch, a root canonical is canonical only in its owning surface). For a **fresh routed handoff awaiting first ingestion**, `-TBI` remains until successful content read is recorded by `-ingested`, or ASK retires the artifact before ingestion through `-supersededA`; for anything else the same read removes `-TBI` only, leaving the underlying filename unchanged. Establish governed role and prior lifecycle state before feeding — where either is unresolved, stop rather than resolve either branch. A metadata-only retrieval is **not** a content read; a lossy or normalized view may be a content read under a bounded-fidelity claim, but never counts as exact-byte evidence and is not adequate where the omitted portion could affect classification.
