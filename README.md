...
The control-surface design recorded in this repo is anchored to twelve pieces of external writing:
...
- [*The Rule the Repo Could Not Contain*](https://atomicspacekitten.substack.com/p/the-rule-the-repo-could-not-contain) — the next workflow piece after *From Conversation to Control Surface*. Names the recursive externality topology that the cluster has been building toward: a public repo can know that an upstream voice and surface-boundary discipline exists, but cannot contain its protected payload — only the structural category lives repo-side. Reframes externality as a relation rather than a property — the same artifact can be external to one surface and internal to another at the same time — and distinguishes two axes (surface boundary; normative direction) so that grounding notes (upstream-external) and hooks (downstream-external) are visibly different even though both are ex-repo. Articulates the invariant across the recursive method ecology: repo-readable surfaces carry rules, categories, cadences, and authority boundaries; grounding notes carry source-of-intent and doctrine; operator-side memory carries substitutable specifics; articles carry reflection on the system. Uses `urban-observatory`'s post-bootstrap trim pass as a worked instance of meta-to-downstream absorption — the meta repo is external to a downstream's specific instance but internal to the method that produced the lesson. The article is itself the principle in action: it operates at the methodology layer above the meta repo it reflects on, and is positioned as a link from the meta repo rather than an absorption into it.
- [*Three Agents Got Into an Argument // The Repo Won*](https://atomicspacekitten.substack.com/p/three-agents-got-into-an-argument) — the next workflow piece after *The Rule the Repo Could Not Contain*. Stress-tests the method on a multi-advisor disagreement about what a file currently contains. Names the operating rule the dispute proved: chat-based advisors are useful for reasoning, synthesis, critique, and framing; the artifact-owning surface is authoritative for current artifact state. Distinguishes three surface-specific failure modes — confident causal confabulation, confident structural confabulation, and narrow surface authority — and connects them to the prior reasoning-failures piece. Names the thematic-tidiness tell: a diagnosis too clean may be doing rhetorical work, not retrieval work. Positioned as a link from the meta repo rather than an absorption into it.
- [*The Handoff Is Not the Instruction*](https://atomicspacekitten.substack.com/p/the-handoff-is-not-the-instruction) — the next workflow piece after *Three Agents Got Into an Argument // The Repo Won*. Names a methodology-layer guardrail at the moment external expertise enters an AI-native workflow: well-formed artifacts — memos, sketches, expert handoffs — structurally tend to be read as current direction, even when they represent exploration, future roadmap, or premature architecture. Articulates a five-category taxonomy for incoming contributions — valid source-of-intent / current-stage information-architecture or method refinement / future roadmap idea / advisor scratch or exploration / premature implementation architecture — so each can be classified by operational status before shaping current work. Names the operating rule: only the first two categories reshape current control-surface work by default; later categories are stored for the stage that will need them, not promoted into method on arrival. Preserves repeatability of the method as the load-bearing constraint. Positioned as a link from the meta repo rather than an absorption into it.
