...
- [`prompts/inter-session-coordination.md`](prompts/inter-session-coordination.md) — coordinator runbook for coordination between independent sessions **inside one operating surface** — notifying a live session that state it depends on changed, or that an explicitly checked dependency remains blocked; cross-surface coordination stays with ASK's relay: durable-owner-first ordering for transition events (a checked-but-still-blocked gate creates no durable write and no artifact), the event vocabulary and notice envelope, the receiver rule, and a dated non-normative runtime binding kept separate from the portable contract
