...
**Why once at the project level, not per thread:** re-pasting the advisor prompt into a Project that already holds a prior version in its thread history makes the model *review the revision* instead of *adopt the role* — a launch misfire the **bootstrap + floor pair** sidesteps, because both are installed at the Project level rather than carried into a thread. The advisor discipline (role + fail-closed connector read-path) is the durable asset; this is a relocation of *where* it lives, not a change to *what* it says.

The former per-thread `advisor-initial-prompt` is deprecated. Its current operative requirements are owned by the advisor-surface registry and the bootstrap template; the Project-Instructions template is only the pre-retrieval floor.
...
- **GPT advisor side** — the critique runs as a **fresh thread inside a separate, empty mirror GPT project** holding no other threads. The mirror carries the **same mounted surface bootstrap** and the **same thin Project Instructions floor** as the working project, and — like the working project — **no mounted index**: the index is fetched live at the bootstrap's locator. The empty mirror project is what guarantees a genuinely fresh advisor thread: a fresh thread inside the working project would still inherit that project's conversation history ambiently. Because the mounted bootstrap carries the contract, the fresh thread takes the critique prompt directly too.

**The unifying rule:** a **filesystem executor** (Claude subagent) reads reliably by construction → straight to the critique prompt; a **connector advisor** (GPT) needs read-path calibration → installed once at the Project level as the mounted bootstrap plus the thin Instructions floor, not re-pasted per thread → then straight to the critique prompt. "Fresh context" = a spawned subagent on the Claude side, an empty-mirror-project fresh thread on the GPT side.
...
So the rule is: **the connector advisor fetches the package source directly when a Dropbox connector is available; otherwise mount/upload it — and if neither reaches it, the advisor declares that leg "not reviewed."** A critique never claims to have reviewed a surface it could not read — the same read-path honesty required for repo files and PRs. Connector access is a read path, not a status change (a fetched scratch file stays scratch), and never browses private personal roots unless ASK names the exact path — see the read-path + wall guard in `templates/advisor-project-bootstrap.template.md`.

**Consequence:** do **not** mount operator-side canonicals into the advisor Project's Sources by default. Where the connector exists, Dropbox is the **preferred live read path**. An advisor Project should normally mount **only the surface bootstrap** as a standing Source; the source index / path map is **fetched live** at the locator the bootstrap declares and is **not** a standing Source, and the canonicals are fetched at the locators the index declares.

Mounting or uploading a package source is a **temporary, task-specific fallback** — for a critique whose package the connector cannot reach or whose path is ambiguous. It is not another standing architecture carrier: retire it once live access returns. Deployment contract: [`advisor-project-surface-architecture.md`](advisor-project-surface-architecture.md).
