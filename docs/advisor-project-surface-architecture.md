...
| LIFE-4h | **Grammar and historical boundary.** The lifecycle suffix is always the final token before `.md`; an **addressee** marker (`-4ASK`, `-4TMK`) precedes it and is never stacked after it. **`-PTX` is on a separate axis** — an artifact-role marker that never combines with `-TBI`, `-ingested`, or any terminal disposition suffix; route a separate handoff rather than stacking them. Ordinary disposition words are lower-case; supersession uses the lower-case `superseded` stem plus the ruled uppercase phase qualifier `A` or `P`. The grammar is **prospective** — historical filenames keep the conventions in force when they were created and are never normalized to match it. | shared protocol | BOOTSTRAP | PRESERVE | n/a (new) | separable: grammar and the no-normalization boundary are independently droppable from a carrier that states the transitions |
