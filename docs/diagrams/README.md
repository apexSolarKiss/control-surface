...
- `_dsa-surface/` — pinned, byte-identical `surface-shell` (CSS + navigation runtime), `surface-panel` and `surface-action` carriers plus the mode-aware ASK wordmark pair, used only by `index.html`.
