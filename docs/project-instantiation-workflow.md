...
- when an external advisor surface is used (GPT or Claude in chat form):
  ...
  - for exact-byte advisor review, the executor publishes a named `-PROPOSED` review object — or, when a single object would exceed what the advisor can retrieve completely or a target is not representable as a text patch, a declared review bundle (ordered exact-object or patch parts, one per target, each hashed) — to the project's mapped shared scratch (`<project-name>-EXTERNAL/scratch/`) and the advisor fetches it there; a reported review object is immutable and a revision takes a new `-PROPOSED` name; manual operator upload is fallback only for content already authorized to the advisor when the mapped path is unreachable, and never to cross a wall (`AGENTS.md` §Advisor-Readable Review Objects; advisor-PI §Verification discipline; `_INDEX` scratch row)
...
- the advisor Project's mounted bootstrap and thin Instructions floor, when an external advisor surface (GPT or Claude in chat form) is used
