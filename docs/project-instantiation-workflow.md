...
- if **any hosted Project surface governed by [`docs/advisor-project-surface-architecture.md`](advisor-project-surface-architecture.md) is planned** — an ASK-facing repo-advisor Project or a hosted domain-authority review Project alike — **decide that exact hosted Project instance's memory scope before the Project is created**, and record it — per instance — with its rationale in that variant's Project configuration record, outside the pasted Instructions fence. Both variants use the same deployment shape and therefore inherit the same requirement; only the carrier differs — a repo-advisor instance records it in the Instructions template's outside-fence section, a domain-authority review instance in its own operator configuration canonical. Personal-context Projects are separately governed across the wall and are out of scope here. The instance is the configuration unit; the role or function it serves is the rationale, and two instances serving one broad role may correctly differ. The current host does not permit changing scope in place, so an unrecorded default is a decision made by omission. A continuity function may choose the host default; a contextual-isolation function chooses Project-only **and** the empty-Project workflow, because Project-only alone does not make a new thread fresh. There is no universally correct value — HOST-1 and A13 own the requirement, and this step is where the decision is made rather than inherited
...
- when an external advisor surface is used (GPT or Claude in chat form):
  ...
  - **for exact-byte advisor review**, the normal repo review object is the **pushed PR** once it exists; in the paired path, **Stage-1 readiness includes the minimum exact review object** — before reporting `exact scoped diff ready for approval`, the executor publishes a named `-PROPOSED` review object — or, when a target is not representable as a text patch, a declared review bundle (ordered exact-object or patch parts, one per target, each hashed) — to the project's **mapped shared scratch** (`<project-name>-EXTERNAL/scratch/`), reporting its exact path, baseline, byte size, and SHA-256, or identifies an already advisor-readable exact surface. ASK's relay of the readiness receipt opens pre-PR advisor review — the relay is the request; **Direct execution requires an explicit ASK authorization** (a `FYI` or `HOLD` envelope suppresses the advisor verdict but authorizes nothing; silence or non-relay elects nothing), and a summary relayed without the object leaves review open with readiness incomplete and one publication request to the executor. A reported review object is immutable and a revision takes a new `-PROPOSED` name. **Retrieval order: exact path → raw bytes → one bounded alternate representation.** Resolving the path proves reachability, not fidelity — extracted text, previews, and normalized views are inspection representations, and **path metadata or a lossy view is not exact-byte availability**. Do not use ASK as a byte courier while exact bytes remain retrievable through the authorized mapped route; **manual operator upload is fallback only** after both rungs have failed and been reported and ASK explicitly elects it, for content already authorized to the advisor, and never to cross a wall (`AGENTS.md` §Advisor-Readable Review Objects; advisor bootstrap §Verification; `_INDEX` scratch row)
