...
Because the fresh-context critique cycle is a heavier instrument than the minimal source-of-intent nudge, its initial critique prompt may carry a structured-style boundary classifier. That is intentional. The minimal nudge preserves organic next-direction synthesis during regular cycles; fresh critique reconstructs the project from durable truth and should report whether it sees missing intent, unresolved means, sequencing ambiguity, repo-local absorption / routing needs, evidence-depth limits, or need for further critique. The structured classifier belongs at this altitude because the cycle's purpose is not merely to continue motion, but to test the project's self-map against durable sources.
