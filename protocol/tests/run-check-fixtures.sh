...
d=$(owner_copy L3en); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*Evidence localization and pointer economy\.\*\*//g' "$d/$f"; done; record "NEG carriers lose the evidence-localization / pointer-economy rule" 1 "throughput-discipline" runlocal "$d"
d=$(owner_copy L3eo); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*Artifact function sets the correction threshold\.\*\*//g' "$d/$f"; done; record "NEG carriers lose the artifact-function correction threshold" 1 "throughput-discipline" runlocal "$d"
d=$(owner_copy L3ep); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*The declared review identity\.\*\*//g' "$d/$f"; done; record "NEG carriers lose the declared-review-identity contract" 1 "throughput-discipline" runlocal "$d"
d=$(owner_copy L3eq); perl -0pi -e 's/\*\*Apply proportionality by artifact function\.\*\*//g' "$d/templates/advisor-project-bootstrap.template.md"; record "NEG advisor-bootstrap loses artifact-function proportionality" 1 "throughput-discipline" runlocal "$d"
d=$(owner_copy L3er); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/report the declared review identity as part of the readiness receipt/report its exact locator, baseline, byte size, and SHA-256 as part of the readiness receipt/g' "$d/$f"; done; record "NEG carriers reintroduce an unconditional review-identity recital" 1 "throughput-discipline" runlocal "$d"
