...
d=$(owner_copy L3cm); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*Evidence localization and pointer economy\.\*\*//g' "$d/$f"; done; record "NEG carriers lose the evidence-localization / pointer-economy rule" 1 "throughput-discipline"
d=$(owner_copy L3cn); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*Artifact function sets the correction threshold\.\*\*//g' "$d/$f"; done; record "NEG carriers lose the artifact-function correction threshold" 1 "throughput-discipline"
d=$(owner_copy L3co); for f in protocol/AGENTS.shared.md AGENTS.md; do perl -0pi -e 's/\*\*Localize review evidence\.\*\*//g' "$d/$f"; done; record "NEG carriers lose review-object evidence localization" 1 "throughput-discipline"
d=$(owner_copy L3cp); perl -0pi -e 's/\*\*Apply proportionality by artifact function\.\*\*//g' "$d/templates/advisor-project-bootstrap.template.md"; record "NEG advisor-bootstrap loses artifact-function proportionality" 1 "throughput-discipline"
